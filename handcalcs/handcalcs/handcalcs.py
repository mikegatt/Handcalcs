...
def format_calc_line(line: CalcLine, **config_options) -> CalcLine:
    ...
    line.latex = f"{comment} {comment_space} {latex_code[0:second_equals + 1]} {latex_code[second_equals + 2:]}\n"
    return line
...
def format_calc_line(line: NumericCalcLine, **config_options) -> NumericCalcLine:
    ...
    if line.comment:
        comment_space = "\\;"
        comment = format_strings(line.comment, comment=True)
    line.latex = f"{comment} {comment_space} {latex_code}\n"
    return line
...
def format_long_calc_line(line: LongCalcLine, **config_options) -> LongCalcLine:
    ...
    if line.comment:
        comment_space = "\\;"
        comment = format_strings(line.comment, comment=True)
    line.latex = f"{comment}{comment_space}{long_latex} {line_break}"
    return line
...
def format_param_line(line: ParameterLine, **config_options) -> ParameterLine:
    ...
    if "=" in line.latex:
        ...
        comment = format_strings(line.comment, comment=True)
        line.latex = f"{comment}{comment_space}{replaced}{line_break}"
...
def format_symbolic_line(line: SymbolicLine, **config_options) -> SymbolicLine:
    ...
    comment_space = "\\;"
    comment = format_strings(line.comment, comment=True)
    line.latex = f"{comment}{comment_space}{replaced}  \n"
    return line
